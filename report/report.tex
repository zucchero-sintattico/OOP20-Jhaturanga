...
\title{Relazione progetto Jhaturanga}
...
Il Team si pone come obiettivo lo sviluppo di un applicativo per giocare al gioco degli scacchi, evoluzione dell'antico gioco indiano "Chaturanga".
%
...
\begin{itemize}
    ...
    \item Differenti obiettivi (farsi dare matto, perdere tutti i pezzi)
    \item Regole differenti per movimento, cattura e scacco
    \item Tipi differenti di pezzi
    \item Scacchiere differenti (dimensioni diverse, cilindriche, sferiche, tridimensionali ecc.)
\end{itemize}
...
\begin{itemize}
    ...
    \item L'applicazione dovrà essere in grado di gestire una classica partita di scacchi, applicando quindi le regole di base di quest'ultimo.
    \item L'utente avrà la possibilità di scegliere il tema dell'applicazione, della griglia e dei pezzi.
    \item Si avrà a disposizione un menu dal quale sarà possibile gestire tutte le impostazioni relative all'applicazione e al profilo dell'utente.
    ...
    \item Il giocatore potrà richiedere la patta all'avversario
    ...
    \item L’applicazione potrà gestire più profili utente mediante un login con password in modo da conservare i dati in maniera permanente tra una sessione di gioco e l’altra.
    \item L’utente  accumulerà  ad  ogni  partita  dei punti  che  verranno  utilizzati per calcolare il suo nuovo punteggio (la cui determinazione sarà simile a quella ELO standard).TODO aggiungi referenza wiki
    \item Al termine della partita si potrà salvare la partita con la possibilità quindi di visualizzarne il replay in un secondo momento.
    \item Evidenziamento delle celle dove è possibile muoversi
    \item Implementazione multiplayer
    \item L'applicazione permetterà di giocare ad alcune varianti scacchistiche come opzione agli scacchi base 
    \item Sarà possibile accedere ad una modalità per la risoluzione di problemi scacchistici
    \item Gestione dei suoni
    \item Si potrà selezionare la lingua 
\end{itemize}

\subsubsection{Requisiti non funzionali}
\begin{itemize}
	\item L’applicazione dovrà garantire una sicurezza minima sul salvataggio delle password di ogni utente tramite funzioni di hashing.
	\item La serializzazione delle partite per il relativo salvataggio su file e il successivamento caricamento dovranno essere effettuate in modo efficiente riducendo al minimo le informazioni che andranno salvate.
